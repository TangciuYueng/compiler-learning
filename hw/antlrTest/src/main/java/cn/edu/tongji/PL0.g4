...
constant_declaration : CONST constant_definition (COMMA constant_definition)* SEMI ;
// <常量定义> -> <标识符>:=<无符号整数>
constant_definition : identifier ASSIGN unsigned_integer ;
// <无符号整数> -> <数字>{<数字>}
unsigned_integer : NUM ;
// <变量说明> -> VAR<标识符>{,<标识符>};
variable_declaration : VAR identifier (COMMA identifier)* SEMI ;
...
// <因子> -> <标识符>|<无符号整数>|(表达式)
factor : identifier | unsigned_integer | LPAREN expression RPAREN ;
// <条件语句> -> IF <条件> THEN <语句>
